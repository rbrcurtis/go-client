...
(function($) {
	$.prompt = function(message, options) {
		...
		$.each(message,function(statename,stateobj){
			...
			$state.children('.'+ $.prompt.options.prefix +'buttons').children('button').on('tap', function(){
		...
		$.prompt.jqif.on('click', fadeClicked);
		$window.resize({animate:false}, $.prompt.position);
		$.prompt.jqi.find('.'+ $.prompt.options.prefix +'close').on('click', $.prompt.close);
